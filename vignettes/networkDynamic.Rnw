...
\author{Ayn Leslie-Cook, Zack Almquist, Skye Bender-deMoll,\\ Martina Morris, Carter T. Butts}
\maketitle
...
<<newcomb,include=TRUE,echo=TRUE>>=options(keep.source = TRUE, width = 60)
require(networkDynamic)
data(newcomb)             # load the data
names(newcomb)            # its some kind of named list object
length(newcomb$networks)   # how many networks?
is.network(newcomb$networks[[1]]) # is it really a network?
as.sociomatrix(newcomb$networks[[1]]) # peek at sociomatrix       
newcombDyn <- ndConverter(newcomb$networks)  # make dynamic  
# check that it is the same
identical(as.sociomatrix(newcomb$networks[[1]]),
          as.sociomatrix(network.extract(newcombDyn,onset=0,terminus=1)))
@
When converting panel data in this form, \verb@ndConverter@ assumes that the panels should be assigned times of unit intervals, so the first panel is given the spell [0,1), the second [1,2), etc. 

\subsection{Converting from a data.frame of spells}
The \verb@ndConverter@ function can create network dynamic objects from data.frames. It assumes that the first two columns correspond to the network indices of the ego and alter vertices, and the third and forth columns give the onset and terminus of a spell for that dyad. Multiple spells per dyad are expressed by multiple rows. In the following example, we read some tabular data describing arc relationships out of an example text file. (The text file is in the .son format, \verb@?read.son@ for information on importing the entire file.)
<<file_spells,include=TRUE,echo=TRUE>>=options(keep.source = TRUE, width = 60)
spellData <-read.table(system.file('extdata/cls33_10_16_96.son', 
            package='networkDynamic'),skip=22,header=T)
spellData[1:5,] # peek
classDyn <- ndConverter(spellData[,1:4]) #just grab 4 cols
@
...
<<newcomb,include=TRUE,echo=TRUE>>=options(keep.source = TRUE, width = 60)
newcombEdgeSpells<-ndConverter(newcombDyn,format='edgelist')
dim(newcombEdgeSpells) # how big is it?
newcombEdgeSpells[1:5,] # peek at the beginning
@
The first two columns of the spell matrix gives the network indices of the vertices involved in the edge, and the next two give the onset and terminus for the spell.  The \verb@Noncensored@ column indicates if a statistical estimation process using this spell list should assume that the the entire duration of the edge's activity is included or if it was partially censored by the observation.  The \verb@duration@ column gives the total duration for the specific spell of the edge, not the entire edge duration. 
...
<<classroom_binning,include=TRUE,echo=TRUE>>=options(keep.source = TRUE, width = 60)
